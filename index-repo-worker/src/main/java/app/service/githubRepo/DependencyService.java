...
public class DependencyService {
    ...
    public CompletableFuture<Map<String, List<Dependency>>> fetchRepoDependencies(
        ...
        try {
            ...
            for (String language : DependencyFileEnum.getUniqueLanguages()) {
                Optional<List<Dependency>> languageDependencies = fetchLanguageDeps(repo, language);
                if (languageDependencies.isEmpty()) continue;

                dependenciesByLanguage.put(language, null);
            }
    ...
    private Optional<List<Dependency>> fetchLanguageDeps(GHRepository repo, String language) throws JsonProcessingException {
        for (DependencyFileEnum nonLockFile : DependencyFileEnum.getNonLockFilesForLanguage(language)) {
            Optional<String> nonLockFileContent = tryFetchFileContent(repo, nonLockFile);
            if (nonLockFileContent.isEmpty()) continue;

            Optional<String> lockFileContent = nonLockFile.getLockFiles().stream()
                .map(lockfile -> tryFetchFileContent(repo, lockfile))
                .filter(opt -> opt.isPresent())
                .map(opt -> opt.get())
                .findFirst();
            
            return Optional.of(dependencyParserStrategy.parse(
                nonLockFile, nonLockFileContent.get(), lockFileContent.orElse(null)));
        }
        return Optional.empty();
    }

    private Optional<String> tryFetchFileContent(GHRepository repo, DependencyFileEnum file) {
        try {
            GHContent ghContent = repo.getFileContent(file.path);
